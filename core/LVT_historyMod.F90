    ...
    if(trim(LVT_rc%lvt_out_format).eq."netcdf") then 
      ...
      if(LVT_rc%lvt_wopt.eq."1d tilespace") then 
         do t=1,LVT_LIS_rc(1)%ntiles
            ...
            LVT_histData%xlat%value(t,1) = LVT_domain%grid(gid)%lat
            LVT_histData%xlon%value(t,1) = LVT_domain%grid(gid)%lon
         enddo
      else
      ! set lat/lons 
         do t=1,LVT_rc%ngrid
            LVT_histData%xlat%value(t,1) = LVT_domain%grid(t)%lat
            LVT_histData%xlon%value(t,1) = LVT_domain%grid(t)%lon
         enddo
      endif
      ...
      do r=1,LVT_rc%gnr
         do c=1,LVT_rc%gnc
            ...
            if(gid.ne.-1) then 
               gtmp(c,r) = LVT_histData%xlat%value(gid,1)
            endif
         enddo
      enddo
      ...
      do r=1,LVT_rc%gnr
         do c=1,LVT_rc%gnc
            gid = LVT_domain%gindex(c,r)
            if(gid.ne.-1) then 
               gtmp(c,r) = LVT_histData%xlon%value(gid,1)
            endif
         enddo
      enddo
...
!BOP
! 
! !ROUTINE: LVT_voidLISModelData
! \label(LVT_voidLISModelData)
!
! !INTERFACE:
  subroutine LVT_voidLISModelData
! 
! !USES:   
    implicit none
!
...
!EOP
    ...
    if(LVT_rc%anlys_data_class.eq."LSM") then 
       dataEntry => LVT_histData%head_lsm_ds1_list
    elseif(LVT_rc%anlys_data_class.eq."Routing") then 
       dataEntry => LVT_histData%head_routing_ds1_list
    elseif(LVT_rc%anlys_data_class.eq."RTM") then 
       dataEntry => LVT_histData%head_rtm_ds1_list
    elseif(LVT_rc%anlys_data_class.eq."Irrigation") then 
       dataEntry => LVT_histData%head_irrig_ds1_list
    endif
    
    do while(associated(dataEntry))
       call voidSingleVar(dataEntry)
       dataEntry => dataEntry%next
    enddo
  end subroutine LVT_voidLISModelData

!BOP
! 
! !ROUTINE: voidSingleVar
! \label(voidSingleVar)
!
! !INTERFACE:
  subroutine voidSingleVar(dataEntry)
! 
! !USES:   
...
!EOP
    ...
    do k=dataEntry%startNlevs,dataEntry%endNlevs
       dataEntry%value(:,k) = LVT_rc%udef
       dataEntry%count(:,k) = 0 
    enddo
  end subroutine voidSingleVar
#endif
!BOP
! 
