...
!EOP
  ...
  type(pctiledec), save :: LVT_pctile_struc
  private
...
! 
! !INTERFACE: 
  subroutine LVT_initpercentile(model, obs, stats,metric)
! !ARGUMENTS: 
...
!EOP
    ...
    if(LVT_metrics%percentile%selectOpt.eq.1.or.&
       ...
       LVT_pctile_struc%d_classes(1) = 0.30
       LVT_pctile_struc%d_classes(2) = 0.20
       LVT_pctile_struc%d_classes(3) = 0.10
       LVT_pctile_struc%d_classes(4) = 0.05
       LVT_pctile_struc%d_classes(5) = 0.02

       allocate(stats%pctile%value_model_nsize(LVT_rc%ngrid, &
            365))
       stats%pctile%value_model_nsize = 0 
       
       allocate(stats%pctile%value_model(LVT_rc%ngrid,model%selectNlevs,1))
       stats%pctile%value_model  = 0
       allocate(stats%pctile%value_count_model(LVT_rc%ngrid,model%selectNlevs,1))
       stats%pctile%value_count_model  = 0

       allocate(stats%pctile%tavg_value_model(LVT_rc%ngrid,model%selectNlevs,1))
       stats%pctile%tavg_value_model  = 0
       allocate(stats%pctile%tavg_count_value_model(LVT_rc%ngrid,model%selectNlevs,1))
       stats%pctile%tavg_count_value_model  = 0

       allocate(stats%pctile%value_model_ci(model%selectNlevs, 1))

       if(LVT_rc%tavgInterval.ne.86400) then
          write(LVT_logunit,*) 'Percentile computations are only supported'
          write(LVT_logunit,*) 'with a daily average interval...'
          call LVT_endrun()
       endif
       LVT_pctile_struc%nwindow = 5
       ...
       if(LVT_rc%startmode.eq."coldstart") then 
          ...
          call LVT_computeTimeSpanInYears(LVT_pctile_struc%nyears)
          
          LVT_pctile_struc%nsize_total = LVT_pctile_struc%nwindow * & 
               LVT_pctile_struc%nyears
          
          allocate(pctile_model(LVT_rc%ngrid,&
               LVT_pctile_struc%nsize_total))
          ...
          do k=1,365
             ...
             write(ftn) LVT_pctile_struc%nsize_total
          ...
          do k=1,365
             ...
             ftn = LVT_getNextUnitNumber()
             open(ftn,file=trim(filename),form='unformatted')
             read(ftn) LVT_pctile_struc%nsize_total
             ...
             do t=1,LVT_rc%ngrid
                call compute_sorted_sizes(&
                     LVT_pctile_struc%nsize_total,&
                     pctile_model(t,:),&
                     stats%pctile%value_model_nsize(t,k))
             enddo
          enddo
       endif
...
!-------------------------------------------------------------------------
       ...
       allocate(LVT_pctile_struc%ftn_ts_darea(LVT_rc%ntslocs))
       ...
       do i=1,LVT_rc%ntslocs
          ...
          LVT_pctile_struc%ftn_ts_darea(i) = LVT_getNextUnitNumber()

          open(LVT_pctile_struc%ftn_ts_darea(i),file=(filename),&
               form='formatted')
...
!-------------------------------------------------------------------------
       ...
       if(LVT_pctile_struc%usescaling.eq.1) then 
          call ESMF_ConfigGetAttribute(LVT_config,&
               LVT_pctile_struc%mean_filename1,&
          ...
          call ESMF_ConfigGetAttribute(LVT_config,&
               LVT_pctile_struc%std_filename1,&
          ...
          call ESMF_ConfigGetAttribute(LVT_config,&
               LVT_pctile_struc%mean_filename2,&
          ...
          call ESMF_ConfigGetAttribute(LVT_config,&
               LVT_pctile_struc%std_filename2,&
          ...
          call ESMF_TimeSet(LVT_pctile_struc%stime, &
          ...
          enddo
          call ESMF_TimeSet(LVT_pctile_struc%etime, &
               yy=etime(1), mm=etime(2), dd=etime(3),&
               h = etime(4), m = etime(5), s = etime(6), &
               calendar=LVT_calendar, rc=rc)
          call LVT_verify(rc, 'error in ESMF_TimeSet: end time')

          allocate(LVT_pctile_struc%mean1(LVT_rc%ngrid))
          allocate(LVT_pctile_struc%mean2(LVT_rc%ngrid))
          allocate(LVT_pctile_struc%std1(LVT_rc%ngrid))
          allocate(LVT_pctile_struc%std2(LVT_rc%ngrid))

          write(LVT_logunit,*) 'Reading mean file ',&
               trim(LVT_pctile_struc%mean_filename1)
          ftn = LVT_getNextUnitNumber()
          open(ftn,file=LVT_pctile_struc%mean_filename1, form='unformatted')
          read(ftn) var
          call LVT_releaseUnitNumber(ftn)
          ...
          do r=1,LVT_rc%lnr
             do c=1,LVT_rc%lnc
                if(LVT_domain%gindex(c,r).ne.-1) then 
                   LVT_pctile_struc%mean1(LVT_domain%gindex(c,r)) = var(c,r) 
                endif
             enddo
          enddo

          write(LVT_logunit,*) 'Reading mean file ',&
               trim(LVT_pctile_struc%mean_filename2)
          ftn = LVT_getNextUnitNumber()
          open(ftn,file=LVT_pctile_struc%mean_filename2, form='unformatted')
          read(ftn) var
          call LVT_releaseUnitNumber(ftn)
          ...
          do r=1,LVT_rc%lnr
             do c=1,LVT_rc%lnc
                if(LVT_domain%gindex(c,r).ne.-1) then 
                   LVT_pctile_struc%mean2(LVT_domain%gindex(c,r)) = var(c,r) 
                endif
             enddo
          enddo

          write(LVT_logunit,*) 'Reading standard deviation file ',&
               trim(LVT_pctile_struc%std_filename1)
          ftn = LVT_getNextUnitNumber()
          open(ftn,file=LVT_pctile_struc%std_filename1, form='unformatted')
          read(ftn) var
          call LVT_releaseUnitNumber(ftn)
          ...
          do r=1,LVT_rc%lnr
             do c=1,LVT_rc%lnc
                if(LVT_domain%gindex(c,r).ne.-1) then 
                   LVT_pctile_struc%std1(LVT_domain%gindex(c,r)) = var(c,r) 
                endif
             enddo
          enddo

          write(LVT_logunit,*) 'Reading standard deviation file ',&
               trim(LVT_pctile_struc%std_filename2)
          ftn = LVT_getNextUnitNumber()
          open(ftn,file=LVT_pctile_struc%std_filename2, form='unformatted')
          read(ftn) var
          call LVT_releaseUnitNumber(ftn)
          ...
          do r=1,LVT_rc%lnr
             do c=1,LVT_rc%lnc
                if(LVT_domain%gindex(c,r).ne.-1) then 
                   LVT_pctile_struc%std2(LVT_domain%gindex(c,r)) = var(c,r) 
                endif
             enddo
          enddo
       endif
...
!EOP
    ...
    real                    :: min_value
    real                    :: m_value
    real                    :: pctile_model_day1(LVT_rc%ngrid,&
         LVT_pctile_struc%nsize_total)
    real                    :: pctile_model_day2(LVT_rc%ngrid,&
         LVT_pctile_struc%nsize_total)
    real                    :: pctile_model_day3(LVT_rc%ngrid,&
         LVT_pctile_struc%nsize_total)
    real                    :: pctile_model_day4(LVT_rc%ngrid,&
         LVT_pctile_struc%nsize_total)
    real                    :: pctile_model_day5(LVT_rc%ngrid,&
         LVT_pctile_struc%nsize_total)
    ...
    if(stats%selectOpt.eq.1.and.&
       ...
       if(.not.skip_flag) then 
          ...
          p1  = (yr_index -1)*LVT_pctile_struc%nwindow +1
          p2  = (yr_index -1)*LVT_pctile_struc%nwindow +2
          p3  = (yr_index -1)*LVT_pctile_struc%nwindow +3
          p4  = (yr_index -1)*LVT_pctile_struc%nwindow +4
          p5  = (yr_index -1)*LVT_pctile_struc%nwindow +5
...
!ignore if the data goes outside the allotted time window
                      ...
                      if(LVT_pctile_struc%usescaling.eq.1.and.&
                           currTime.ge.LVT_pctile_struc%stime.and.&
                           currTime.le.LVT_pctile_struc%etime) then 
                         m_value = LVT_pctile_struc%mean2(t) + &
                              (model%value(t,1) - LVT_pctile_struc%mean1(t))* &
                              (LVT_pctile_struc%std2(t)/&
                              LVT_pctile_struc%std1(t))
!                         tmp1(LVT_domain%grid(t)%col,LVT_domain%grid(t)%row) = m_value
!                         tmp2(LVT_domain%grid(t)%col,LVT_domain%grid(t)%row) = model%value(t,1)
                      else
                         m_value = model%value(t,1)
                      endif
                         
                      if(p1.le.LVT_pctile_struc%nsize_total) then 
                         pctile_model_day1(t,p1)  = & 
                              m_value
                      endif
                      
                      
                      if(p2.le.LVT_pctile_struc%nsize_total) then 
                         pctile_model_day2(t,p2)   = & 
                              m_value
                      endif

                      if(p3.le.LVT_pctile_struc%nsize_total) then 
                         pctile_model_day3(t,p3)   = & 
                              m_value
                      endif


                      if(p4.le.LVT_pctile_struc%nsize_total) then 
                         pctile_model_day4(t,p4) = &
                              m_value
                      endif

                      if(p5.le.LVT_pctile_struc%nsize_total) then 
          ...
          do t=1,LVT_rc%ngrid
             if(model%value(t,1).gt.min_value) then 
                ...
                call percentile_value(&
                     m_value,&
                     stats%pctile%value_model_nsize(t,tindex),&                
                     pctile_model(t,:),&
                     pvalue)
                stats%pctile%value_model(t,1,1) = & 
                     stats%pctile%value_model(t,1,1) + pvalue
                stats%pctile%value_count_model(t,1,1) = &
                     stats%pctile%value_count_model(t,1,1) +1
             endif
          enddo
       endif
    endif
          ...
          if(LVT_metrics%percentile%selectOpt.eq.1.or.&
             ...
             if(alarm) then 
                if(LVT_metrics%percentile%timeOpt.eq.1.and.&
                   ...
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%percentile%ftn_ts_loc(i),&
                           200,advance='no') &
                           LVT_rc%yr, '',LVT_rc%mo, '', LVT_rc%da, '', &
                           LVT_rc%hr,'',LVT_rc%mn, '' 
                      write(LVT_pctile_struc%ftn_ts_darea(i),&
                           200,advance='no') &
                           LVT_rc%yr, '',LVT_rc%mo, '', LVT_rc%da, '', &
                           LVT_rc%hr,'',LVT_rc%mn, '' 
                   enddo
...
200          format(I4, a1, I2.2, a1, I2.2, a1, I2.2, a1, I2.2,a1)
             ...
             if(alarm) then 
                if(LVT_metrics%percentile%timeOpt.eq.1.and.&
                     LVT_metrics%percentile%extractTS.eq.1) then 
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%percentile%ftn_ts_loc(i),fmt='(a1)') ''
                      write(LVT_pctile_struc%ftn_ts_darea(i),fmt='(a1)') ''
                   enddo
                endif
             endif
          endif
...
!EOP
    ...
    integer  :: count_percentile_model(LVT_rc%ngrid,model%selectNlevs,1)
    integer  :: count_percentile_obs(LVT_rc%ngrid,obs%vlevels,1)
    ...
    if(stats%selectOpt.eq.1.and.&
       ...
       do t=1,LVT_rc%ngrid
          do k=1,model%selectNlevs
             if(stats%pctile%value_count_model(t,k,1).gt.0) then 
                stats%pctile%value_model(t,k,1) = &
                     stats%pctile%value_model(t,k,1)/&
                     stats%pctile%value_count_model(t,k,1)

                stats%pctile%tavg_value_model(t,k,1) = & 
                     stats%pctile%tavg_value_model(t,k,1) + & 
                     stats%pctile%value_model(t,k,1) 
                stats%pctile%tavg_count_value_model(t,k,1) = &
                     stats%pctile%tavg_count_value_model(t,k,1) + 1
             else
                stats%pctile%value_model(t,k,1) = LVT_rc%udef
             endif
          enddo
       enddo
       ...
       if(alarm) then 
          ...
          do t=1,LVT_rc%ngrid
             do k=1,model%selectNlevs
                if(stats%pctile%tavg_count_value_model(t,k,1).gt.0) then 
                   stats%pctile%tavg_value_model(t,k,1) = & 
                        stats%pctile%tavg_value_model(t,k,1) /&
                        stats%pctile%tavg_count_value_model(t,k,1)  
                else
                   stats%pctile%tavg_value_model(t,k,1) = LVT_rc%udef
                endif
             enddo
          enddo
          ...
          if(metric%extractTS.eq.1) then 
             call LVT_writeTSinfo(metric%ftn_ts_loc,&
                  model,&
                  LVT_rc%ngrid,&
                  stats%pctile%tavg_value_model,&
                  stats%pctile%tavg_count_value_model)
          endif
       endif

       do k=1,model%selectNlevs
          call LVT_computeCI(stats%pctile%tavg_value_model(:,k,1),&
               LVT_rc%ngrid,&
               LVT_rc%pval_CI, stats%pctile%value_model_ci(k,1))
       enddo

          
       do k=1,model%selectNlevs
          call LVT_writeDataBasedStrat(model,obs,stats,metric,&
               LVT_rc%ngrid, stats%pctile%tavg_value_model)
       enddo
          ...
          do i=1,LVT_rc%ntslocs
             ...
             do kk=1,LVT_TSobj(i)%npts
                ...
                if(tid.ne.-1) then 
                   ...
                   do m=1,1
                      t = stid+m
                      if(stats%pctile%tavg_value_model(t,k,1).ne.LVT_rc%udef) then 
                         if(stats%pctile%tavg_value_model(t,k,1).lt.&
                              LVT_pctile_struc%d_classes(1)) then 
                            sumv(1,i) = sumv(1,i) + 1
                         endif
                         if(stats%pctile%tavg_value_model(t,k,1).lt.&
                              LVT_pctile_struc%d_classes(2)) then 
                            sumv(2,i) = sumv(2,i) + 1
                         endif
                         if(stats%pctile%tavg_value_model(t,k,1).lt.&
                              LVT_pctile_struc%d_classes(3)) then 
                            sumv(3,i) = sumv(3,i) + 1
                         endif
                         if(stats%pctile%tavg_value_model(t,k,1).lt.&
                              LVT_pctile_struc%d_classes(4)) then 
                            sumv(4,i) = sumv(4,i) + 1
                         endif
                         if(stats%pctile%tavg_value_model(t,k,1).lt.&
                              LVT_pctile_struc%d_classes(5)) then 
                            sumv(5,i) = sumv(5,i) + 1
                         endif
                      endif
                   enddo
       ...
       if(alarm) then 
          do i=1,LVT_rc%ntslocs
             write(LVT_pctile_struc%ftn_ts_darea(i),203,advance='no') &
              ...
              do k=1,vlevels
                if(LVT_metrics%percentile%timeOpt.eq.1) then 
                   call LVT_writevar_gridded(LVT_metrics%percentile%ftn_ts, &
                        stats%pctile%tavg_value_model(:,k,1),&
...
!EOP
    ...
    do while(associated(model))
       if(LVT_metrics%percentile%selectOpt.eq.1) then 
          do l=1,LVT_pctile_struc%nsize_total
             call LVT_writevar_restart(ftn,&
                  LVT_pctile_struc%model_final(:,l),tileflag=1)
          enddo
          do l=1,LVT_pctile_struc%nasc
             call LVT_writevar_restart(ftn,&
                  LVT_pctile_struc%model_mu(:,l),tileflag=1)
             call LVT_writevar_restart(ftn,&
                  LVT_pctile_struc%model_sigma(:,l),tileflag=1)
          enddo
       end if
