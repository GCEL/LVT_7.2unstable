          ...
          do k=1,model%selectNlevs
             ...
             if(trim(obs%units).eq.trim(model%units)) then 
                if(model%count(t,m_k).ne.0) then 
                   if(model%value(t,m_k).gt.stats%kstest%value_model_max(t,k)) then 
                      stats%kstest%value_model_max(t,k) = &
                           model%value(t,m_k)
                   endif
                   if(model%value(t,m_k).lt.stats%kstest%value_model_min(t,k)) then 
                      stats%kstest%value_model_min(t,k) = &
                           model%value(t,m_k)
                   endif
                endif
                if(obs%count(t,o_k).ne.0) then  
                   if(obs%value(t,o_k).gt.stats%kstest%value_obs_max(t,k)) then 
                      stats%kstest%value_obs_max(t,k) = &
                           obs%value(t,o_k)
                   endif
                   if(obs%value(t,o_k).lt.stats%kstest%value_obs_min(t,k)) then 
