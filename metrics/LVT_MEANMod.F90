...
!EOP
    ...
    if(metric%timeOpt.eq.1) then 
       ...
       if(LVT_rc%obssource(2).ne."none") then 
          if(obs%selectNlevs.ge.1) then 
             allocate(stats%mean%obs_value_ts(LVT_rc%ngrid, obs%selectNlevs, &
                  LVT_rc%strat_nlevels))
             stats%mean%obs_value_ts = 0.0
             allocate(stats%mean%count_obs_value_ts(LVT_rc%ngrid, obs%selectNlevs, &
                  LVT_rc%strat_nlevels))
             stats%mean%count_obs_value_ts = 0


             allocate(stats%mean%tavg_obs_value_ts(LVT_rc%ngrid, obs%selectNlevs, &
                  LVT_rc%strat_nlevels))
             stats%mean%tavg_obs_value_ts = 0.0
             allocate(stats%mean%tavg_count_obs_value_ts(LVT_rc%ngrid, obs%selectNlevs, &
                  LVT_rc%strat_nlevels))
             stats%mean%tavg_count_obs_value_ts = 0

             if(obs%stdev_flag) then 
                allocate(stats%mean%obs_value_stdev_ts(LVT_rc%ngrid, obs%selectNlevs, &
                  LVT_rc%strat_nlevels))
                stats%mean%obs_value_stdev_ts = 0.0
                allocate(stats%mean%count_obs_value_stdev_ts(LVT_rc%ngrid, obs%selectNlevs, &
                     LVT_rc%strat_nlevels))
                stats%mean%count_obs_value_stdev_ts = 0

                allocate(stats%mean%tavg_obs_value_stdev_ts(LVT_rc%ngrid, obs%selectNlevs, &
                  LVT_rc%strat_nlevels))
                stats%mean%tavg_obs_value_stdev_ts = 0.0
                allocate(stats%mean%tavg_count_obs_value_stdev_ts(LVT_rc%ngrid, obs%selectNlevs, &
                     LVT_rc%strat_nlevels))
       ...
       do t=1,LVT_rc%ngrid
          do k=1,model%selectNlevs
             ...
             if(model%count(t,m_k).gt.0) then
                ...
                if(metric%timeOpt.eq.1) then 
                   if(model%value(t,m_k).ne.LVT_rc%udef) then 
                      stats%mean%model_value_ts(t,k,1) = &
                           stats%mean%model_value_ts(t,k,1)+&
                           model%value(t,m_k)
       ...
       if(stats%selectOpt.eq.1.and.&
          ...
          if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
             call LVT_writeDataBasedStrat(model,obs,stats,metric,&
                  ...
                  LVT_rc%ngrid,stats%mean%obs_value_total)
             
             if(metric%computeSC.eq.1) then 
                call LVT_writeSeasonalCycleInfo(model,obs,stats,metric,&
                     LVT_rc%ngrid,stats%mean%model_value_asc,&
                     stats%mean%count_model_value_asc,&
                     LVT_rc%ngrid,stats%mean%obs_value_asc,&
                     stats%mean%count_obs_value_asc)          
             endif
             if(metric%computeADC.eq.1) then 
                call LVT_writeAvgDiurnalCycleInfo(model,obs,stats,metric,&
                     LVT_rc%ngrid,stats%mean%model_value_adc,&
                     stats%mean%count_model_value_adc,&
                     LVT_rc%ngrid,stats%mean%obs_value_adc,&
                     stats%mean%count_obs_value_adc)     
             endif
          else
             call LVT_writeDataBasedStrat(model,obs,stats,metric,&
                   LVT_rc%ngrid,stats%mean%model_value_total)
