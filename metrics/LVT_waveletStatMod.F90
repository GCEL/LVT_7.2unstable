...
contains
  ...
  subroutine LVT_initwaveletStat(model, obs, stats,metric)
! !ARGUMENTS: 
...
!EOP
    if(metric%selectOpt.eq.1) then 
       allocate(stats%wvt%value_model_mean_total(LVT_rc%ngrid, model%selectNlevs, &
            LVT_rc%strat_nlevels))
       stats%wvt%value_model_mean_total = 0.0
       allocate(stats%wvt%count_value_model_mean_total(LVT_rc%ngrid, model%selectNlevs, &
            LVT_rc%strat_nlevels))
       stats%wvt%count_value_model_mean_total = 0
       
       if(LVT_rc%obssource(2).ne."none") then 
          if(obs%selectNlevs.ge.1) then 
             allocate(stats%wvt%value_obs_mean_total(LVT_rc%ngrid, obs%vlevels, &
                  LVT_rc%strat_nlevels))
             stats%wvt%value_obs_mean_total = 0.0
             allocate(stats%wvt%count_value_obs_mean_total(LVT_rc%ngrid, &
                  obs%vlevels, &
                  LVT_rc%strat_nlevels))
             stats%wvt%count_value_obs_mean_total = 0
          endif
       endif
       allocate(stats%wvt%value_mse_total(LVT_rc%nscales))
       allocate(stats%wvt%value_mse_pct_total(LVT_rc%nscales))
    endif
    if(metric%timeOpt.eq.1) then 
       allocate(stats%wvt%value_model_mean_ts(LVT_rc%ngrid, model%selectNlevs, &
            LVT_rc%strat_nlevels))
       stats%wvt%value_model_mean_ts = 0.0
       allocate(stats%wvt%count_value_model_mean_ts(LVT_rc%ngrid, model%selectNlevs, &
            LVT_rc%strat_nlevels))
       stats%wvt%count_value_model_mean_ts = 0
       
       if(LVT_rc%obssource(2).ne."none") then 
          if(obs%selectNlevs.ge.1) then 
             allocate(stats%wvt%value_obs_mean_ts(LVT_rc%ngrid, model%selectNlevs, &
                  LVT_rc%strat_nlevels))
             stats%wvt%value_obs_mean_ts = 0.0
             allocate(stats%wvt%count_value_obs_mean_ts(LVT_rc%ngrid, model%selectNlevs, &
                  LVT_rc%strat_nlevels))
             stats%wvt%count_value_obs_mean_ts = 0
          endif
       endif
       allocate(stats%wvt%value_mse_ts(LVT_rc%nscales))
       allocate(stats%wvt%value_mse_pct_ts(LVT_rc%nscales))
             ...
             if(trim(obs%units).eq.trim(model%units)) then 
                if(metric%selectOpt.eq.1) then
                   if(model%value(t,m_k).ne.LVT_rc%udef) then 
                      stats%wvt%value_model_mean_total(t,k,1) = &
                           stats%wvt%value_model_mean_total(t,k,1) + &
                           model%value(t,m_k)
                      stats%wvt%count_value_model_mean_total(t,k,1) = &
                           stats%wvt%count_value_model_mean_total(t,k,1) + 1
                   endif
                   if(obs%value(t,o_k).ne.LVT_rc%udef) then 
                      stats%wvt%value_obs_mean_total(t,k,1) = &
                           stats%wvt%value_obs_mean_total(t,k,1) + &
                           obs%value(t,o_k)
                      stats%wvt%count_value_obs_mean_total(t,k,1) = &
                           stats%wvt%count_value_obs_mean_total(t,k,1) + 1
                   endif
                   if(metric%timeOpt.eq.1) then 
                      if(model%value(t,m_k).ne.LVT_rc%udef) then 
                         stats%wvt%value_model_mean_ts(t,k,1) = &
                              stats%wvt%value_model_mean_ts(t,k,1)+&
                              model%value(t,m_k)
                         stats%wvt%count_value_model_mean_ts(t,k,1) = & 
                              stats%wvt%count_value_model_mean_ts(t,k,1)+1
                      endif
                      if(obs%value(t,o_k).ne.LVT_rc%udef) then 
                         stats%wvt%value_obs_mean_ts(t,k,1) = &
                              stats%wvt%value_obs_mean_ts(t,k,1)+&
                              obs%value(t,o_k)
                         stats%wvt%count_value_obs_mean_ts(t,k,1) = & 
                              stats%wvt%count_value_obs_mean_ts(t,k,1)+1
                      endif
                   endif
                endif
             else
          ...
          if(alarm) then 
             if(LVT_metrics%waveletStat%timeOpt.eq.1.and.&
                ...
                do i=1,LVT_rc%ntslocs
                   write(LVT_metrics%waveletStat%ftn_ts_loc(i),200,advance='no') &
...
200       format(I4, a1, I2.2, a1, I2.2, a1, I2.2, a1, I2.2,a1)
          ...
          if(alarm) then 
             if(LVT_metrics%waveletStat%timeOpt.eq.1.and.&
                  LVT_metrics%waveletStat%extractTS.eq.1) then 
                do i=1,LVT_rc%ntslocs
                   write(LVT_metrics%waveletStat%ftn_ts_loc(i),fmt='(a1)') ''
                enddo
             endif
          endif
       endif
    ...
    if(metric%timeOpt.eq.1.and.alarm) then 
       if(stats%selectOpt.eq.1.and.obs%selectNlevs.ge.1) then 
          do t=1,LVT_rc%ngrid
             do k=1,model%selectNlevs
                do l=1,LVT_rc%strat_nlevels
                   if(stats%wvt%count_value_model_mean_ts(t,k,l).ne.0) then 
                      stats%wvt%value_model_mean_ts(t,k,l) = &
                           stats%wvt%value_model_mean_ts(t,k,l)/&
                           stats%wvt%count_value_model_mean_ts(t,k,l) 
                   else
                      stats%wvt%value_model_mean_ts(t,k,l) = LVT_rc%udef
                   endif

                   if(stats%wvt%count_value_obs_mean_ts(t,k,l).ne.0) then 
                      stats%wvt%value_obs_mean_ts(t,k,l) = &
                           stats%wvt%value_obs_mean_ts(t,k,l)/&
                           stats%wvt%count_value_obs_mean_ts(t,k,l) 
                   else
                      stats%wvt%value_obs_mean_ts(t,k,l) = LVT_rc%udef
                   endif
          ...
          do k=1,model%selectNlevs
             do l=1,LVT_rc%strat_nlevels
                do t=1,LVT_rc%ngrid
                   if(stats%wvt%value_model_mean_ts(t,k,l).gt.metric%threshold) then 
                      Ix(t) = 1
                   else
                      Ix(t) = 0
                   endif
                   if(stats%wvt%value_obs_mean_ts(t,k,l).gt.metric%threshold) then 
                      Iy(t) = 1
                   else
                      Iy(t) = 0
                   endif
                   ...
                   Zval(t) = Iy(t)-Ix(t)
                enddo
                call waveletScaleDecomp(Zval, stats%wvt%value_mse_ts, &
                     stats%wvt%value_mse_pct_ts)
             enddo
          enddo
          ...
          if(metric%extractTS.eq.1) then 
             do i=1, LVT_rc%ntslocs
                do k=1,LVT_rc%nscales
                   write(metric%ftn_ts_loc(i),203,advance='no') &
                        stats%wvt%value_mse_ts(k), stats%wvt%value_mse_pct_ts(k)
                enddo
             enddo
          endif
       endif
    ...
    if(LVT_rc%endtime.eq.1.and.metric%selectOpt.eq.1) then 
       if(stats%selectOpt.eq.1.and.obs%selectNlevs.ge.1) then
          do t=1,LVT_rc%ngrid
             do k=1,model%selectNlevs
                do l=1,LVT_rc%strat_nlevels
                   if(stats%wvt%count_value_model_mean_total(t,k,l).ne.0) then 
                      stats%wvt%value_model_mean_total(t,k,l) = &
                           stats%wvt%value_model_mean_total(t,k,l)/&
                           stats%wvt%count_value_model_mean_total(t,k,l) 
                   else
                      stats%wvt%value_model_mean_total(t,k,l) = LVT_rc%udef
                   endif

                   if(stats%wvt%count_value_obs_mean_total(t,k,l).ne.0) then 
                      stats%wvt%value_obs_mean_total(t,k,l) = &
                           stats%wvt%value_obs_mean_total(t,k,l)/&
                           stats%wvt%count_value_obs_mean_total(t,k,l) 
                   else
                      stats%wvt%value_obs_mean_total(t,k,l) = LVT_rc%udef
                   endif
          ...
          do k=1,model%selectNlevs
             do l=1,LVT_rc%strat_nlevels
                do t=1,LVT_rc%ngrid
                   if(stats%wvt%value_model_mean_total(t,k,l).gt.metric%threshold) then 
                      Ix(t) = 1
                   else
                      Ix(t) = 0
                   endif

                   if(stats%wvt%value_obs_mean_total(t,k,l).gt.metric%threshold) then 
                      Iy(t) = 1
                   else
                      Iy(t) = 0
                   endif
                ...
                call waveletScaleDecomp(Zval, stats%wvt%value_mse_total,&
                     stats%wvt%value_mse_pct_total)
             enddo
          enddo
             ...
             if(pass.eq.1) then 
                if(stats%selectOpt.eq.1) then 
                   do i=1, LVT_rc%ntslocs
                      do k=1,LVT_rc%nscales
                         write(ftn,204) &
                              k, stats%wvt%value_mse_total(k), &
                              stats%wvt%value_mse_pct_total(k)
                      enddo
                   enddo
                endif
             endif
       ...
       if(stats%selectOpt.eq.1) then 
          do k=1,model%selectNlevs
             if(LVT_metrics%waveletstat%timeOpt.eq.1) then 
                do l=1,LVT_rc%strat_nlevels
                   stats%wvt%value_model_mean_ts(:,k,l) = 0.0
                   stats%wvt%count_value_model_mean_ts(:,k,l)=0 
                enddo
                if(LVT_rc%obssource(2).ne."none".and.&
                     obs%selectNlevs.ge.1) then 
                   do l=1,LVT_rc%strat_nlevels
                      stats%wvt%value_obs_mean_ts(:,k,l) = 0.0
                      stats%wvt%count_value_obs_mean_ts(:,k,l)=0 
                   enddo
                endif
             endif
          enddo
