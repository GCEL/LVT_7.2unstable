...
!   code needs to store the precip values in time to compute SSWI, 
...
! 
! !INTERFACE: 
  subroutine LVT_initSSWI(model, obs, stats,metric)
! !ARGUMENTS: 
...
!EOP
    ...
    if(LVT_metrics%sswi%selectOpt.eq.1.or.&
       ...
       if((model%short_name.eq."RootMoist").or.&
          ...
          call LVT_parseTimeString(time,LVT_sswi_struc%tscale)
          LVT_sswi_struc%tscale = LVT_sswi_struc%tscale/2592000          

          if(LVT_rc%tavgInterval.ne.2592000) then 
             write(LVT_logunit,*) 'SSWI values are computed at a monthly timescale'
          ...
          LVT_sswi_struc%nasc = 12
          
          call computeNmonths(LVT_sswi_struc%nsize_total, LVT_rc%tavgInterval)
          LVT_sswi_struc%nsize_season = &
               LVT_sswi_struc%nsize_total/LVT_sswi_struc%nasc
          
          if(metric%selectOpt.eq.1) then 
             allocate(LVT_sswi_struc%model_rzsm_final(LVT_rc%ngrid,&
                  LVT_sswi_struc%nsize_total))
             LVT_sswi_struc%model_rzsm_final = 0.0
             
             allocate(LVT_sswi_struc%model_mu(LVT_rc%ngrid,&
                  LVT_sswi_struc%nasc))
             allocate(LVT_sswi_struc%model_sigma(LVT_rc%ngrid,&
                  LVT_sswi_struc%nasc))
             allocate(LVT_sswi_struc%model_sswi(LVT_rc%ngrid))
             LVT_sswi_struc%model_sswi = LVT_rc%udef

             allocate(LVT_sswi_struc%model_current(LVT_rc%ngrid,&
                  LVT_sswi_struc%tscale))
             LVT_sswi_struc%model_current = LVT_rc%udef
             
             if(LVT_rc%obssource(2).ne."none") then 
                if(obs%selectNlevs.ge.1) then 
                   allocate(LVT_sswi_struc%obs_rzsm_final(LVT_rc%ngrid,&
                        LVT_sswi_struc%nsize_total))
                   LVT_sswi_struc%obs_rzsm_final = 0.0
                   
                   allocate(LVT_sswi_struc%obs_mu(LVT_rc%ngrid,&
                        LVT_sswi_struc%nasc))
                   allocate(LVT_sswi_struc%obs_sigma(LVT_rc%ngrid,&
                        LVT_sswi_struc%nasc))
                   allocate(LVT_sswi_struc%obs_sswi(LVT_rc%ngrid))
                   LVT_sswi_struc%obs_sswi = LVT_rc%udef
                  
                   allocate(LVT_sswi_struc%obs_current(LVT_rc%ngrid,1))
                   LVT_sswi_struc%obs_current = LVT_rc%udef
             ...
             if(model%count(t,m_k).gt.0) then
                if(metric%selectOpt.eq.1) then 
                   if(model%value(t,m_k).ne.LVT_rc%udef) then 
                      LVT_sswi_struc%model_rzsm_final(t,tindex_f) = &
                           model%value(t,m_k)
                   endif
                endif
             endif
                ...
                if(obs%selectNlevs.ge.1) then
                   if(obs%count(t,o_k).gt.0) then 
                      if(metric%selectOpt.eq.1.and.&
                         ...
                         LVT_sswi_struc%obs_rzsm_final(t,tindex_f) = &
                              obs%value(t,o_k)
                      endif
             ...
             if(alarm) then 
                if(LVT_metrics%sswi%timeOpt.eq.1.and.&
                   ...
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%sswi%ftn_ts_loc(i),200,advance='no') &
...
200          format(I4, a1, I2.2, a1, I2.2, a1, I2.2, a1, I2.2,a1)
             ...
             if(alarm) then 
                if(LVT_metrics%sswi%timeOpt.eq.1.and.&
                     LVT_metrics%sswi%extractTS.eq.1) then 
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%sswi%ftn_ts_loc(i),fmt='(a1)') ''
                   enddo
                endif
             endif
          endif
...
!EOP
    ...
    if(LVT_rc%endtime.eq.1.and.metric%selectOpt.eq.1) then 
       if(stats%selectOpt.eq.1.and.&
          ...
          do t=1,LVT_rc%ngrid
             do k=1,model%selectNlevs
                do l=1,LVT_sswi_struc%nasc
                   call sswi_normal(t,l,LVT_sswi_struc%nsize_total,&
                        LVT_sswi_struc%nsize_season,&
                        LVT_sswi_struc%model_rzsm_final(t,:),&
                        LVT_sswi_struc%model_mu(t,l),&
                        LVT_sswi_struc%model_sigma(t,l))
                enddo
             enddo
          enddo
          do t=1,LVT_rc%ngrid
                ...
                if(LVT_rc%obssource(2).ne."none") then 
                   if(obs%selectNlevs.ge.1) then 
                      do l=1,LVT_sswi_struc%nasc
                         call sswi_normal(t,l,LVT_sswi_struc%nsize_total,&
                              LVT_sswi_struc%nsize_season,&
                              LVT_sswi_struc%obs_rzsm_final(t,:),&
                              LVT_sswi_struc%obs_mu(t,l),&
                              LVT_sswi_struc%obs_sigma(t,l))
                      enddo
                   endif
                endif
             enddo
...
! 
! !INTERFACE: 
  subroutine sswi_normal(t,l,nsize,nsize_season,rzsm, mu, sigma)
...
!EOP  
    ...
    rzsm_season = 0.0
    k = 0 
    do i=l,nsize,LVT_sswi_struc%nasc
       k = k+1

       tmprzsm = 0.0
       rzsm_count = 0 
       do m=1,LVT_sswi_struc%tscale
          t1_ind = (i-LVT_sswi_struc%tscale+m)
          if(t1_ind.gt.0) then 
...
!EOP
    ...
    if(stats%selectOpt.eq.1.and.&
       ...
       do m=LVT_sswi_struc%tscale,1,-1
          if(m.ne.1) then 
             LVT_sswi_struc%model_current(:,m) = &
                  LVT_sswi_struc%model_current(:,m-1)
          else
             LVT_sswi_struc%model_current(:,m) = & 
                  model%value(:,1)
          endif
       ...
       do t=1,LVT_rc%ngrid
          ...
          do m=1,LVT_sswi_struc%tscale
             if(LVT_sswi_struc%model_current(t,m).ne.LVT_rc%udef) then 
                tmpval = tmpval + LVT_sswi_struc%model_current(t,m)
          ...
          call sswi_zval(t,&
               tmpval,&
               LVT_sswi_struc%model_mu(t,tind), &
               LVT_sswi_struc%model_sigma(t,tind), &
               LVT_sswi_struc%model_sswi(t))
       ...
       if(LVT_rc%obssource(2).ne."none") then  
          if(obs%selectNlevs.ge.1) then 
             ...
             do m=LVT_sswi_struc%tscale,1,-1
                if(m.ne.1) then 
                   LVT_sswi_struc%obs_current(:,m) = &
                        LVT_sswi_struc%obs_current(:,m-1)
                else
                   LVT_sswi_struc%obs_current(:,m) = & 
                        obs%value(:,1)
                endif
             ...
             do t=1,LVT_rc%ngrid
                ...
                do m=1,LVT_sswi_struc%tscale
                   if(LVT_sswi_struc%obs_current(t,m).ne.LVT_rc%udef) then 
                      tmpval = tmpval + LVT_sswi_struc%obs_current(t,m)
                      val_count = val_count + 1
                   endif
                enddo
                if(val_count.gt.0) then 
                   ...
                   call sswi_zval(t,&
                        tmpval,&
                        LVT_sswi_struc%obs_mu(t,tind), &
                        LVT_sswi_struc%obs_sigma(t,tind), &
                        LVT_sswi_struc%obs_sswi(t))
                else
                   LVT_sswi_struc%obs_sswi(t) = LVT_rc%udef
                endif
             enddo
       ...
       if(metric%extractTS.eq.1) then 
          model_sswi(:,1,1) = LVT_sswi_struc%model_sswi(:)
          if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
             obs_sswi(:,1,1) = LVT_sswi_struc%model_sswi(:)
       ...
       call LVT_computeCI(LVT_sswi_struc%model_sswi(:),LVT_rc%ngrid,&
            LVT_rc%pval_CI, LVT_sswi_struc%model_sswi_ci(1,1))
       
       if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
          call LVT_computeCI(LVT_sswi_struc%obs_sswi(:),LVT_rc%ngrid,&
               LVT_rc%pval_CI,LVT_sswi_struc%obs_sswi_ci(1,1))
       endif
       ...
       if(final.ne.1) then
           if(stats%selectOpt.eq.1) then 
              do k=1,vlevels
                if(LVT_metrics%sswi%timeOpt.eq.1) then 
                   call LVT_writevar_gridded(LVT_metrics%sswi%ftn_ts, &
                        LVT_sswi_struc%model_sswi(:),&
                        stats%vid_ts(LVT_SSWIid,1),1)
                   if(LVT_rc%obssource(2).ne."none") then 
                      call LVT_writevar_gridded(LVT_metrics%sswi%ftn_ts, &
                           LVT_sswi_struc%obs_sswi(:),&
             ...
             call LVT_writeSummaryStats(&
                  ...
                  count_model_sswi,&
                  stats%short_name,&
                  LVT_sswi_struc%model_sswi_ci)
             if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
                obs_sswi(:,1,1) = LVT_sswi_struc%obs_sswi(:)
                call LVT_writeSummaryStats(&
                     LVT_metrics%sswi%ftn_summ,&
                     LVT_metrics%sswi%short_name,&
                     LVT_rc%ngrid,&
          ...
          if(LVT_metrics%sswi%selectOpt.eq.1) then 
             do l=1,LVT_sswi_struc%nasc
                call LVT_writevar_restart(ftn,&
                     LVT_sswi_struc%model_mu(:,l))
                call LVT_writevar_restart(ftn,&
                     LVT_sswi_struc%model_sigma(:,l))
             enddo
             if(LVT_rc%obssource(2).ne."none") then 
                do l=1,LVT_sswi_struc%nasc
                   call LVT_writevar_restart(ftn,&
                        LVT_sswi_struc%obs_mu(:,l))
                   call LVT_writevar_restart(ftn,&
                        LVT_sswi_struc%obs_sigma(:,l))
                enddo
             endif
          end if
       endif
