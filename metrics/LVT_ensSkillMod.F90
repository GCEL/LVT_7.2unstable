          ...
          do g=1,LVT_rc%ngrid
             do k=1, model%selectNlevs
                ...
                do m=1,LVT_LIS_rc(1)%nensem
                   ...
                   if(obs%count(g,o_k).ne.0.and. &
                        model%count(g,m_k).ne.0) then      
                      if(metric%selectOpt.eq.1) then 
                         if(model%value(g,m_k).ne.LVT_rc%udef) then 
                            sumval = sumval + model%value(g,m_k)
                            nsumval = nsumval + 1
                         endif
                      endif
                   endif
                ...
                if(nsumval.gt.0) then 
                   ...
                   skill = (ens_mean - obs%value(g,o_k))**2
