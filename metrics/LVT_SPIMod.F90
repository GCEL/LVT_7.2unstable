...
!EOP
    ...
    if(LVT_metrics%spi%selectOpt.eq.1.or.&
       ...
       if(trim(model%short_name).eq."TotalPrecip".or.&
          ...
          call LVT_parseTimeString(time,LVT_spi_struc%tscale)
          LVT_spi_struc%tscale = LVT_spi_struc%tscale/2592000
          ...
          LVT_spi_struc%nsize_season = &
               LVT_spi_struc%nsize_total/LVT_spi_struc%nasc
          ...
          if(metric%selectOpt.eq.1) then 
             ...
             allocate(LVT_spi_struc%model_current(LVT_rc%ngrid,&
                  LVT_spi_struc%tscale))
             LVT_spi_struc%model_current = LVT_rc%udef
             ...
             if(LVT_rc%obssource(2).ne."none") then 
                if(obs%selectNlevs.ge.1) then 
                   ...
                   allocate(LVT_spi_struc%obs_current(LVT_rc%ngrid,1))
                   LVT_spi_struc%obs_current = LVT_rc%udef
...
!    real test1(10), rval
    ...
    do i=l,nsize,LVT_spi_struc%nasc
       ...
       tmppcp = 0.0
       pcp_count = 0 
       do m=1,LVT_spi_struc%tscale
          t1_ind = (i-LVT_spi_struc%tscale+m)
          if(t1_ind.gt.0) then 
...
!EOP
    ...
    if(stats%selectOpt.eq.1.and.&
       ...
       do m=LVT_spi_struc%tscale,1,-1
          if(m.ne.1) then 
             LVT_spi_struc%model_current(:,m) = &
                  LVT_spi_struc%model_current(:,m-1)
          else
             LVT_spi_struc%model_current(:,m) = & 
                  model%value(:,1)
          endif
       ...
       do t=1,LVT_rc%ngrid
          ...
          do m=1,LVT_spi_struc%tscale
             if(LVT_spi_struc%model_current(t,m).ne.LVT_rc%udef) then 
                tmppcp = tmppcp + LVT_spi_struc%model_current(t,m)
                pcp_count = pcp_count + 1
             endif
          enddo
       ...
       if(LVT_rc%obssource(2).ne."none") then 
          if(obs%selectNlevs.ge.1) then 
             ...
             do m=LVT_spi_struc%tscale,1,-1
                if(m.ne.1) then 
                   LVT_spi_struc%obs_current(:,m) = &
                        LVT_spi_struc%obs_current(:,m-1)
                else
                   LVT_spi_struc%obs_current(:,m) = & 
                        obs%value(:,1)
                endif
             ...
             do t=1,LVT_rc%ngrid
                ...
                do m=1,LVT_spi_struc%tscale
                   if(LVT_spi_struc%obs_current(t,m).ne.LVT_rc%udef) then 
                      tmppcp = tmppcp + LVT_spi_struc%obs_current(t,m)
                      pcp_count = pcp_count + 1
                   endif
                enddo
                tmppcp = tmppcp/pcp_count
