...
!-----------------------------------------------------------------------------
  ...
  type(ecomplexitydec), save :: LVT_ecomplexity_struc
...
!-------------------------------------------------------------------------
    ...
    allocate(LVT_ecomplexity_struc%nts(LVT_rc%ngrid, model%selectNlevs))
             ...
             if(model%count(t,m_k).gt.0) then
                if(metric%selectOpt.eq.1) then 
                   if(model%value(t,m_k).ne.LVT_rc%udef) then
                      ...
                      stats%ecomplexity%value_model_ts(t,k,tindex) = model%value(t,m_k)
                   endif
                endif
             endif
          enddo
...
!number of words in the string : N-L+1
    ...
    if(LVT_rc%endtime.eq.1.and.metric%selectOpt.eq.1) then 
       if(stats%selectOpt.eq.1.and.&
          ...
          do t=1,LVT_rc%ngrid
             do k=1,model%selectNlevs
                call trimTimeSeries (t,k,stats%ecomplexity%value_model_ts(t,k,:))
                if(LVT_ecomplexity_struc%nts(t,k).gt.LVT_ecomplexity_struc%nts_threshold) then 
                   call fillTimeSeries (t,k,LVT_ecomplexity_struc%value_model_ts)
                endif
                
                if(LVT_ecomplexity_struc%nts(t,k).le.LVT_ecomplexity_struc%nts_threshold) then 
                   ts_class(t,k) = 0
                else
                   do kk=1,LVT_ecomplexity_struc%nts(t,k)
                      if(LVT_ecomplexity_struc%value_model_ts(kk).eq.LVT_rc%udef) then 
                         ts_class(t,k) = 0
                      endif
                   enddo
                endif
                ...
                if(ts_class(t,k).eq.1) then 
                   med_val(t,k) = median(LVT_ecomplexity_struc%value_model_ts(:),&
                        LVT_ecomplexity_struc%nts(t,k))
                else
                   med_val(t,k) = LVT_rc%udef
                endif

                if(LVT_ecomplexity_struc%nts(t,k).gt.LVT_ecomplexity_struc%nts_threshold) then 
                   allocate(bitstr(LVT_ecomplexity_struc%nts(t,k)))
                   allocate(words(LVT_ecomplexity_struc%nts(t,k)-&
                        LVT_ICwordlength+1,LVT_ICwordlength))
                   
                   if(ts_class(t,k).eq.1) then 
                      do l=1,LVT_ecomplexity_struc%nts(t,k)
                         if(LVT_ecomplexity_struc%value_model_ts(l).ne.LVT_rc%udef) then 
                            if(LVT_ecomplexity_struc%value_model_ts(l).gt.med_val(t,k)) then 
                               bitstr(l) = '1'
                            else
                               bitstr(l) = '0'
                            endif
                   ...
                   if(ts_class(t,k).eq.1) then 
                      nbinsij = 0 
                      nbins = 0 
                      do kk=1,LVT_ecomplexity_struc%nts(t,k)-LVT_ICwordlength+1
...
!conditional probability from the ith to the jth word, given ith word
                      ...
                      stats%ecomplexity%value_final(t,k) = LVT_rc%udef
                   endif
                   deallocate(bitstr)
                   deallocate(words)
                else
  ...
  subroutine trimTimeSeries( t, l,ts_value)
    ...
    st_index = -1
    en_index = -1
    LVT_ecomplexity_struc%nts(t,l) = -1
    do k=1,LVT_rc%nts
    ...
    if(st_index.gt.0.and.en_index.gt.0) then 
       LVT_ecomplexity_struc%nts(t,l) = en_index - st_index + 1
    endif

    if(LVT_ecomplexity_struc%nts(t,l).gt.LVT_ecomplexity_struc%nts_threshold) then 
       if(allocated(LVT_ecomplexity_struc%value_model_ts)) then 
          deallocate(LVT_ecomplexity_struc%value_model_ts)
       endif
       allocate(LVT_ecomplexity_struc%value_model_ts(LVT_ecomplexity_struc%nts(t,l)))
       
       do k=st_index, en_index
          LVT_ecomplexity_struc%value_model_ts(k-st_index+1) =&
               ts_value(k)
       enddo
    endif
  ...
  subroutine fillTimeSeries( t, l, ts_value)
    ...
    integer         :: t,l
    real            :: ts_value(LVT_ecomplexity_struc%nts(t,l))
    integer         :: k, k_new, k_new1, k_new2

    do k=1,LVT_ecomplexity_struc%nts(t,l)
       if(ts_value(k).eq.LVT_rc%udef) then 
          call findNearestIndex2d(t, l, k,ts_value,k_new1, k_new2)
          if(k_new1.gt.0.and.k_new2.gt.0) then 
             if(((k-k_new1).le.LVT_ecomplexity_struc%ngap_threshold)&
                  .and.((k_new2-k).le.LVT_ecomplexity_struc%ngap_threshold)) then 
  ...
  subroutine findNearestIndex2d(tile, level, k, ts_value, k_new1, k_new2)
    ...
    integer         :: tile, level
    real            :: ts_value(LVT_ecomplexity_struc%nts(tile, level))
    integer         :: k, k_new, k_new1, k_new2
    integer         :: t
    ...
    do t=k,LVT_ecomplexity_struc%nts(tile,level)
       if(ts_value(t).ne.LVT_rc%udef) then 
          k_new2 = t
          exit
       endif
