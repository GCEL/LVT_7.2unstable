...
!-----------------------------------------------------------------------------
  ...
  type(mentropydec), save :: LVT_mentropy_struc
...
!-------------------------------------------------------------------------
    ...
    allocate(LVT_mentropy_struc%nts(LVT_rc%ngrid, model%selectNlevs))
             ...
             if(model%count(t,m_k).gt.0) then
                if(metric%selectOpt.eq.1) then 
                   if(model%value(t,m_k).ne.LVT_rc%udef) then
                      ...
                      stats%mentropy%value_model_ts(t,k,tindex) = model%value(t,m_k)
...
! in the cold start mode, write the time series. Else, simply read the gap filled data. 
          ...
          print*, LVT_rc%ngrid, LVT_rc%nts
             ...
             open(ftn,file=LVT_mentropy_struc%rstFile,form='unformatted',&
                  access='direct',recl=LVT_rc%ngrid*LVT_rc%nts*4)
             write(ftn,rec=1) stats%mentropy%value_model_ts(:,1,:)
             ...
             matlab_command='matlab -nosplash -nodisplay -nojvm -r "filldata(' & 
                  ...
                  //"'"//trim(LVT_mentropy_struc%rstFile)//"'"//') "'
...
! Run the Matlab program
          ...
          open(ftn,file=LVT_mentropy_struc%rstFile,form='unformatted',&
               access='direct',recl=LVT_rc%ngrid*LVT_rc%nts*4)
          read(ftn,rec=1) stats%mentropy%value_model_ts(:,1,:)
          call LVT_releaseUnitNumber(ftn)          
          
          do t=1,LVT_rc%ngrid
             do k=1,model%selectNlevs
                call trimTimeSeries (t,k,stats%mentropy%value_model_ts(t,k,:))
                
                if(LVT_mentropy_struc%nts(t,k).le.LVT_mentropy_struc%nts_threshold) then 
                   ts_class(t,k) = 0
                else
                   do kk=1,LVT_mentropy_struc%nts(t,k)
                      if(LVT_mentropy_struc%value_model_ts(kk).eq.LVT_rc%udef) then 
                         ts_class(t,k) = 0
                      endif
                   enddo
                endif
                ...
                if(ts_class(t,k).eq.1) then 
                   med_val(t,k) = median(LVT_mentropy_struc%value_model_ts(:),&
                        LVT_mentropy_struc%nts(t,k))
                else
                   med_val(t,k) = LVT_rc%udef
                endif

                if(LVT_mentropy_struc%nts(t,k).gt.LVT_mentropy_struc%nts_threshold) then 
                   allocate(bitstr(LVT_mentropy_struc%nts(t,k)))
                   allocate(words(LVT_mentropy_struc%nts(t,k)-&
                        LVT_ICwordlength+1,LVT_ICwordlength))
                   
                   if(ts_class(t,k).eq.1) then 
                      do l=1,LVT_mentropy_struc%nts(t,k)
                         if(LVT_mentropy_struc%value_model_ts(l).ne.LVT_rc%udef) then 
                            if(LVT_mentropy_struc%value_model_ts(l).gt.med_val(t,k)) then 
                               bitstr(l) = '1'
                            else
                               bitstr(l) = '0'
                            endif
                   ...
                   if(ts_class(t,k).eq.1) then 
                      nbins = 0 
                      do kk=1,LVT_mentropy_struc%nts(t,k)-LVT_ICwordlength+1
                      ...
                      stats%mentropy%value_final(t,k) = LVT_rc%udef
                   endif
                   deallocate(bitstr)
                   deallocate(words)
                else
          ...
          do t=1,LVT_rc%ngrid
             do k=1,model%selectNlevs
                call trimTimeSeries (t,k,stats%mentropy%value_model_ts(t,k,:))
                if(LVT_mentropy_struc%nts(t,k).gt.LVT_mentropy_struc%nts_threshold) then 
                   call fillTimeSeries (t,k,LVT_mentropy_struc%value_model_ts)
  ...
  subroutine trimTimeSeries( t, l,ts_value)
    ...
    if(st_index.gt.0.and.en_index.gt.0) then 
       LVT_mentropy_struc%nts(t,l) = en_index - st_index + 1
    endif

    if(LVT_mentropy_struc%nts(t,l).gt.LVT_mentropy_struc%nts_threshold) then 
       if(allocated(LVT_mentropy_struc%value_model_ts)) then 
          deallocate(LVT_mentropy_struc%value_model_ts)
       endif
       allocate(LVT_mentropy_struc%value_model_ts(LVT_mentropy_struc%nts(t,l)))
       
       do k=st_index, en_index
          LVT_mentropy_struc%value_model_ts(k-st_index+1) =&
               ts_value(k)
       enddo
    endif
  ...
  subroutine fillTimeSeries( t, l, ts_value)
    ...
    integer         :: t,l
    real            :: ts_value(LVT_mentropy_struc%nts(t,l))
...
!    if(filloption.eq."Linear") then 
       do k=1,LVT_mentropy_struc%nts(t,l)
          if(ts_value(k).eq.LVT_rc%udef) then 
             call findNearestIndex2d(t, l, k,ts_value,k_new1, k_new2)
             if(k_new1.gt.0.and.k_new2.gt.0) then 
                if(((k-k_new1).le.LVT_mentropy_struc%ngap_threshold)&
                     .and.((k_new2-k).le.LVT_mentropy_struc%ngap_threshold)) then 
...
! first check to see if the gaps are within predefined thresholds. 
       do k=1,LVT_mentropy_struc%nts(t,l)
          if(ts_value(k).eq.LVT_rc%udef) then 
             call findNearestIndex2d(t, l, k,ts_value,k_new1, k_new2)
             if(((k-k_new1).gt.LVT_mentropy_struc%ngap_threshold)&
                  .or.((k_new2-k).gt.LVT_mentropy_struc%ngap_threshold)) then 
                fillcheck = .false.
                exit
             endif
          endif
       ...
       if(fillcheck) then 
          ...
          allocate(temp_data(LVT_mentropy_struc%nts(t,l)))
...
!    endif
  ...
  subroutine findNearestIndex2d(tile, level, k, ts_value, k_new1, k_new2)
    ...
    integer         :: tile, level
    real            :: ts_value(LVT_mentropy_struc%nts(tile, level))
    integer         :: k, k_new, k_new1, k_new2
    integer         :: t
    ...
    do t=k,LVT_mentropy_struc%nts(tile,level)
       if(ts_value(t).ne.LVT_rc%udef) then 
          k_new2 = t
          exit
       endif
