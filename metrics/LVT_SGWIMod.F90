...
! 
! !INTERFACE: 
  subroutine LVT_initSGWI(model, obs, stats,metric)
! !ARGUMENTS: 
...
!EOP
    ...
    if(LVT_metrics%sgwi%selectOpt.eq.1.or.&
       ...
       if(model%short_name.eq."TWS") then 
          ...
          call computeNmonths(LVT_sgwi_struc%nsize_total, LVT_rc%tavgInterval)
          LVT_sgwi_struc%nsize_season = &
               LVT_sgwi_struc%nsize_total/LVT_sgwi_struc%nasc
          LVT_sgwi_struc%nsize_total = &
               LVT_sgwi_struc%nasc*LVT_sgwi_struc%nsize_season
          
          if(metric%selectOpt.eq.1) then 
             allocate(LVT_sgwi_struc%model_rzsm_final(LVT_LIS_rc(1)%ntiles,&
                  LVT_sgwi_struc%nsize_total))
             LVT_sgwi_struc%model_rzsm_final = 0.0
             
             allocate(LVT_sgwi_struc%model_mu(LVT_LIS_rc(1)%ntiles,&
                  LVT_sgwi_struc%nasc))
             allocate(LVT_sgwi_struc%model_sigma(LVT_LIS_rc(1)%ntiles,&
                  LVT_sgwi_struc%nasc))
             allocate(LVT_sgwi_struc%model_sgwi(LVT_LIS_rc(1)%ntiles))
             LVT_sgwi_struc%model_sgwi = LVT_rc%udef
             
             if(LVT_rc%obssource(2).ne."none") then 
                if(obs%selectNlevs.ge.1) then 
                   allocate(LVT_sgwi_struc%obs_rzsm_final(LVT_rc%ngrid,&
                        LVT_sgwi_struc%nsize_total))
                   LVT_sgwi_struc%obs_rzsm_final = 0.0
                   
                   allocate(LVT_sgwi_struc%obs_mu(LVT_rc%ngrid,&
                        LVT_sgwi_struc%nasc))
                   allocate(LVT_sgwi_struc%obs_sigma(LVT_rc%ngrid,&
                        LVT_sgwi_struc%nasc))
                   allocate(LVT_sgwi_struc%obs_sgwi(LVT_rc%ngrid))
                   LVT_sgwi_struc%obs_sgwi = LVT_rc%udef
             ...
             if(model%count(t,m_k).gt.0) then
                if(metric%selectOpt.eq.1) then 
                   if(model%value(t,m_k).ne.LVT_rc%udef) then 
                      LVT_sgwi_struc%model_rzsm_final(t,tindex_f) = &
                           model%value(t,m_k)
                   endif
                endif
             endif
                ...
                if(obs%selectNlevs.ge.1) then
                   if(obs%count(t,o_k).gt.0) then 
                      if(metric%selectOpt.eq.1.and.&
                         ...
                         LVT_sgwi_struc%obs_rzsm_final(t,tindex_f) = &
                              obs%value(t,o_k)
                      endif
             ...
             if(alarm) then 
                if(LVT_metrics%sgwi%timeOpt.eq.1.and.&
                   ...
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%sgwi%ftn_ts_loc(i),200,advance='no') &
...
200          format(I4, a1, I2.2, a1, I2.2, a1, I2.2, a1, I2.2,a1)
             ...
             if(alarm) then 
                if(LVT_metrics%sgwi%timeOpt.eq.1.and.&
                     LVT_metrics%sgwi%extractTS.eq.1) then 
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%sgwi%ftn_ts_loc(i),fmt='(a1)') ''
                   enddo
                endif
             endif
          endif
...
!EOP
    ...
    integer  :: t,l,k
    real     :: diff_field(LVT_LIS_rc(1)%ntiles)
    ...
    if(LVT_rc%endtime.eq.1.and.metric%selectOpt.eq.1) then 
       if(stats%selectOpt.eq.1.and.&
          ...
          do t=1,LVT_LIS_rc(1)%ntiles
             do k=1,model%selectNlevs
                do l=1,LVT_sgwi_struc%nasc
                   call sgwi_normal(t,l,LVT_sgwi_struc%nsize_total,&
                        LVT_sgwi_struc%nsize_season,&
                        LVT_sgwi_struc%model_rzsm_final(t,:),&
                        LVT_sgwi_struc%model_mu(t,l),&
                        LVT_sgwi_struc%model_sigma(t,l))
                enddo
             enddo
          enddo
          do t=1,LVT_rc%ngrid
                ...
                if(LVT_rc%obssource(2).ne."none") then 
                   if(obs%selectNlevs.ge.1) then 
                      do l=1,LVT_sgwi_struc%nasc
                         call sgwi_normal(t,l,LVT_sgwi_struc%nsize_total,&
                              LVT_sgwi_struc%nsize_season,&
                              LVT_sgwi_struc%obs_rzsm_final(t,:),&
                              LVT_sgwi_struc%obs_mu(t,l),&
                              LVT_sgwi_struc%obs_sigma(t,l))
                      enddo
                   endif
                endif
             enddo
...
! 
! !INTERFACE: 
  subroutine sgwi_normal(t,l,nsize,nsize_season,rzsm, mu, sigma)
...
!EOP  
    ...
    rzsm_season = 0.0
    k = 0 
    do i=l,nsize,LVT_sgwi_struc%nasc
       k = k+1
...
!EOP
    ...
    if(stats%selectOpt.eq.1.and.&
       ...
       do t=1,LVT_LIS_rc(1)%ntiles
          call sgwi_zval(t,&
               LVT_sgwi_struc%model_rzsm_final(t,tindex_f),&
               LVT_sgwi_struc%model_mu(t,tind), &
               LVT_sgwi_struc%model_sigma(t,tind), &
               LVT_sgwi_struc%model_sgwi(t))
       enddo
       do t=1,LVT_rc%ngrid
          if(LVT_rc%obssource(2).ne."none") then 
             if(obs%selectNlevs.ge.1) then 
                call sgwi_zval(t,&
                     LVT_sgwi_struc%obs_rzsm_final(t,tindex_f),&
                     LVT_sgwi_struc%obs_mu(t,tind), &
                     LVT_sgwi_struc%obs_sigma(t,tind), &
                     LVT_sgwi_struc%obs_sgwi(t))
       ...
       if(metric%extractTS.eq.1) then 
          model_sgwi(:,1,1) = LVT_sgwi_struc%model_sgwi(:)
          if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
             obs_sgwi(:,1,1) = LVT_sgwi_struc%model_sgwi(:)
       ...
       call LVT_computeCI(LVT_sgwi_struc%model_sgwi(:),LVT_LIS_rc(1)%ntiles,&
            LVT_rc%pval_CI, LVT_sgwi_struc%model_sgwi_ci(1,1))
       
       if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
          call LVT_computeCI(LVT_sgwi_struc%obs_sgwi(:),LVT_rc%ngrid,&
               LVT_rc%pval_CI,LVT_sgwi_struc%obs_sgwi_ci(1,1))
       endif
       ...
       if(final.ne.1) then
           if(stats%selectOpt.eq.1) then 
              do k=1,vlevels
                if(LVT_metrics%sgwi%timeOpt.eq.1) then 
                   call LVT_writevar_gridded(LVT_metrics%sgwi%ftn_ts, &
                        LVT_sgwi_struc%model_sgwi(:),&
                        stats%vid_ts(LVT_SGWIid,1),1)
                   if(LVT_rc%obssource(2).ne."none") then 
                      call LVT_writevar_gridded(LVT_metrics%sgwi%ftn_ts, &
                           LVT_sgwi_struc%obs_sgwi(:),&
             ...
             call LVT_writeSummaryStats(&
                  ...
                  count_model_sgwi,&
                  stats%short_name,&
                  LVT_sgwi_struc%model_sgwi_ci)
             if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
                obs_sgwi(:,1,1) = LVT_sgwi_struc%obs_sgwi(:)
                call LVT_writeSummaryStats(&
                     LVT_metrics%sgwi%ftn_summ,&
                     LVT_metrics%sgwi%short_name,&
                     LVT_rc%ngrid,&
       ...
       do while(associated(model))
          if(model%short_name.eq."TWS") then 
             if(LVT_metrics%sgwi%selectOpt.eq.1) then 
                do l=1,LVT_sgwi_struc%nsize_total
                   call LVT_writevar_restart(ftn,&
                        LVT_sgwi_struc%model_rzsm_final(:,l),tileflag=1)
                enddo
                do l=1,LVT_sgwi_struc%nasc
                   call LVT_writevar_restart(ftn,&
                        LVT_sgwi_struc%model_mu(:,l),tileflag=1)
                   call LVT_writevar_restart(ftn,&
                        LVT_sgwi_struc%model_sigma(:,l),tileflag=1)
                enddo
                if(LVT_rc%obssource(2).ne."none") then 
                   do l=1,LVT_sgwi_struc%nsize_total
                      call LVT_writevar_restart(ftn,&
                           LVT_sgwi_struc%obs_rzsm_final(:,l))
                   enddo
                   do l=1,LVT_sgwi_struc%nasc
                      call LVT_writevar_restart(ftn,&
                           LVT_sgwi_struc%obs_mu(:,l))
                      call LVT_writevar_restart(ftn,&
                           LVT_sgwi_struc%obs_sigma(:,l))
                   enddo
                endif
             end if
          endif
