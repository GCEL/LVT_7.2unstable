...
!EOP
  ...
  if(SMAP_smobs(source)%startflag.or.alarmCheck) then 
...
!
! !INTERFACE: 
subroutine read_SMAPsm(source, name, smobs)
! 
! !USES: 
...
!  character*100,   parameter    :: sm_field_name = "soil_moisture"
  ...
  call LVT_verify(status, 'Error opening SM field in SMAP file')
  
  call h5dget_space_f(sm_field_id, dataspace, status)
  call LVT_verify(status, 'Error in h5dget_space_f: readSMAPObs')
  
  call h5sselect_hyperslab_f(dataspace, H5S_SELECT_SET_F, &
       start=offset_file, count=count_file, hdferr=status)
  call LVT_verify(status, 'Error setting hyperslab dataspace in readSMAPObs')
  ...
  call h5dread_f(sm_field_id, H5T_NATIVE_REAL,sm_field,dims,status, &
       memspace, dataspace)
  call LVT_verify(status, 'Error extracting SM field from SMAPfile')
  ...
  do r=1,SMAP_smobs(source)%nr
     do c=1,SMAP_smobs(source)%nc        
        sm1d(t) = sm_field(SMAP_smobs(source)%nc-c+1,r)
        if(sm_field(SMAP_smobs(source)%nc-c+1,r).ne.-9999.0) then 
           li(t) = .true.
        endif
        t = t+1
     enddo
