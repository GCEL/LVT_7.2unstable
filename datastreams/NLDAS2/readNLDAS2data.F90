...
!EOP
    ...
    real :: qle(nc*nr),qh(nc*nr),qg(nc*nr)
...
! the average over the previous NLDAS-2 monthly file.  - Mocko
    ...
    if (nldas2data(source)%anlys_data_class.eq."LSM") then
       ...
       if ((LVT_MOC_SOILMOIST(source).ge.1.or.&
            LVT_MOC_ROOTMOIST(source).ge.1).and.(nldas2data(source)%lsm.eq."SAC")) then
          call create_nldas2data_filename(nldas2data(source)%odir,&
               nldas2data(source)%interval,        &
...
!Computing a 1m root zone soil moisture value
                if (sacsmvar(1,c+(r-1)*nc).lt.9.99E+20) then
                  rootmoist(c+(r-1)*nc) =&
                       (0.1*sml1(c+(r-1)*nc) +&
                       0.3*sml2(c+(r-1)*nc) +&
       ...
       do index = 1,nvars
          ...
          if ((pid(index).eq.precip_f_index).and.(LVT_MOC_TOTALPRECIP(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,precip_f)
          endif
          
          if ((pid(index).eq.tair_f_index).and.(LVT_MOC_TAIRFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,tair_f)
          endif
          
          if ((pid(index).eq.swdown_f_index).and.(LVT_MOC_SWDOWNFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,swdown_f)
          endif
          
          if ((pid(index).eq.lwdown_f_index).and.(LVT_MOC_LWDOWNFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,lwdown_f)
          endif
          
          if ((pid(index).eq.pet_f_index).and.(LVT_MOC_PETFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,pet_f)
          endif
          
          if ((pid(index).eq.cape_f_index).and.(LVT_MOC_CAPEFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,cape_f)
          endif
          
          if ((pid(index).eq.psurf_f_index).and.(LVT_MOC_PSURFFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,psurf_f)
          endif
          
          if ((pid(index).eq.qair_f_index).and.(LVT_MOC_QAIRFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,qair_f)
          endif
          
          if ((pid(index).eq.crain_f_index).and.(LVT_MOC_CRAINFFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,crain_f)
          endif
          
          if ((pid(index).eq.nwind_f_index).and.(LVT_MOC_NWINDFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,nwind_f)
          endif
          
          if ((pid(index).eq.ewind_f_index).and.(LVT_MOC_EWINDFORC(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,ewind_f)
          endif
          
          if ((pid(index).eq.fheight_f_index).and.(LVT_MOC_FHEIGHTFORC(source).ge.1)) then
          ...
          if ((pid(index).eq.qle_index).and.((LVT_MOC_QLE(source).ge.1).or.&
               (LVT_MOC_RNET(source).ge.1))) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,qle)
          endif
          
          if ((pid(index).eq.qh_index).and.((LVT_MOC_QH(source).ge.1).or.&
               (LVT_MOC_RNET(source).ge.1))) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,qh)
          endif
          
          if ((pid(index).eq.qg_index).and.((LVT_MOC_QG(source).ge.1).or.&
               (LVT_MOC_RNET(source).ge.1))) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,qg)
          endif
          
          if ((pid(index).eq.snowf_index).and.(LVT_MOC_SNOWF(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,snowf)
          endif
          
          if ((pid(index).eq.rainf_index).and.((LVT_MOC_RAINF(source).ge.1).or.&
          ...
          if ((pid(index).eq.avgsurft_index).and.((LVT_MOC_AVGSURFT(source).ge.1).or.&
               (LVT_MOC_RADT(source).ge.1))) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,avgsurft)
          endif
          
          if ((pid(index).eq.albedo_index).and.(LVT_MOC_ALBEDO(source).ge.1)) then
          ...
          if ((pid(index).eq.snowdepth_index).and.(LVT_MOC_SNOWDEPTH(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,snowdepth)
          endif
          
          if ((pid(index).eq.snowcover_index).and.(LVT_MOC_SNOWCOVER(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,snowcover)
          endif
          
          if ((pid(index).eq.potevap_index).and.(LVT_MOC_POTEVAP(source).ge.1)) then
          ...
          if ((pid(index).eq.canopint_index).and.(LVT_MOC_CANOPINT(source).ge.1)) then
...
! Grab the NLDAS-2 VIC top-1 meter soil moisture directly. - Mocko
             if ((pid(index).eq.sml1_index).and.                  &
          ...
          call LVT_verify(iret,                                   &
               'grib_release failed in readNLDAS2data')
       enddo
    ...
    if (nldas2data(source)%lsm.ne."SAC") then
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_SWNET,source,varfield,vlevel=1,     &
            units="W/m2")
       call interp_nldas2var(source,nc,nr, lwnet, varfield)
       call LVT_logSingleDataStreamVar(LVT_MOC_LWNET,source,varfield,vlevel=1,     &
            units="W/m2")
       call interp_nldas2var(source,nc,nr, qle, varfield)
       call LVT_logSingleDataStreamVar(LVT_MOC_QLE,source,varfield,vlevel=1,       &
            units="W/m2")
       rnet = varfield

       call interp_nldas2var(source,nc,nr, qh, varfield)
       call LVT_logSingleDataStreamVar(LVT_MOC_QH,source,varfield,vlevel=1,        &
            units="W/m2")
       do r=1,LVT_rc%lnr
          do c=1,LVT_rc%lnc
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_QG,source,varfield,vlevel=1,        &
            units="W/m2")
       do r=1,LVT_rc%lnr
          do c=1,LVT_rc%lnc
             if(varfield(c,r).ne.LVT_rc%udef) then 
          ...
          enddo
       enddo
       call LVT_logSingleDataStreamVar(LVT_MOC_RNET,source,rnet,vlevel=1,        &
            units="W/m2")

       call interp_nldas2var(source,nc,nr, avgsurft, varfield)
       call LVT_logSingleDataStreamVar(LVT_MOC_AVGSURFT,source,varfield,vlevel=1,  &
            units="K")
       call LVT_logSingleDataStreamVar(LVT_MOC_RADT,source,varfield,vlevel=1,  &
            units="K")
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_ALBEDO,source,varfield,vlevel=1,    &
            units="-")
    endif
    ...
    call LVT_logSingleDataStreamVar(LVT_MOC_SNOWF,source,varfield,vlevel=1,        &
         units="kg/m2s")
    if ((LVT_MOC_TOTALPRECIP(source).ge.1).or.(LVT_MOC_RAINF(source).ge.1)) then
       do r = 1,nr
          do c = 1,nc
    ...
    call LVT_logSingleDataStreamVar(LVT_MOC_RAINF,source,varfield,vlevel=1,        &
         units="kg/m2s")
    call LVT_logSingleDataStreamVar(LVT_MOC_TOTALPRECIP,source,varfield,vlevel=1,        &
         units="kg/m2s")
    ...
    call LVT_logSingleDataStreamVar(LVT_MOC_EVAP,source,varfield,vlevel=1,         &
         units="kg/m2s")
    ...
    call LVT_logSingleDataStreamVar(LVT_MOC_QS,source,qs_ip,vlevel=1,           &
         units="kg/m2s")
    ...
    call LVT_logSingleDataStreamVar(LVT_MOC_QSB,source,qsb_ip,vlevel=1,          &
         units="kg/m2s")
    do r=1,LVT_rc%lnr
       do c=1,LVT_rc%lnc
       ...
       enddo
    enddo
    call LVT_logSingleDataStreamVar(LVT_MOC_RUNOFF,source,trunoff_ip,vlevel=1,  &
         units="kg/m2s")
    ...
    if(LVT_MOC_ECANOPOVERQLE(source).ge.1) then 
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_ECANOPOVERQLE,source,varfield,vlevel=1,          &
            units="-")
    endif
    ...
    if(LVT_MOC_TVEGOVERQLE(source).ge.1) then 
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_TVEGOVERQLE,source,varfield,vlevel=1,          &
            units="-")
    endif
    ...
    if(LVT_MOC_ESOILOVERQLE(source).ge.1) then 
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_ESOILOVERQLE,source,varfield,vlevel=1,          &
            units="-")
    endif
    
    call interp_nldas2var(source,nc,nr, qsm, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_QSM,source,varfield,vlevel=1,          &
         units="kg/m2s")
    call interp_nldas2var(source,nc,nr, swe, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_SWE,source,varfield,vlevel=1,          &
         units="kg/m2")
    call interp_nldas2var(source,nc,nr, snowdepth, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_SNOWDEPTH,source,varfield,vlevel=1,    &
         units="m")
    call interp_nldas2var(source,nc,nr, snowcover,  varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_SNOWCOVER,source,varfield,vlevel=1,    &
         units="-")
    call interp_nldas2var(source,nc,nr, potevap,  varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_POTEVAP,source,varfield,vlevel=1,      &
         units="W/m2")
    call interp_nldas2var(source,nc,nr, ecanop, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_ECANOP,source,varfield,vlevel=1,       &
         units="W/m2")
    call interp_nldas2var(source,nc,nr, tveg, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_TVEG,source,varfield,vlevel=1,         &
         units="W/m2")
    call interp_nldas2var(source,nc,nr, esoil, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_ESOIL,source,varfield,vlevel=1,        &
         units="W/m2")
    call interp_nldas2var(source,nc,nr, canopint, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_CANOPINT,source,varfield,vlevel=1,     &
         units="kg/m2")
    call interp_nldas2var(source,nc,nr, subsnow,  varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_SUBSNOW,source,varfield,vlevel=1,      &
         units="W/m2")
    call interp_nldas2var(source,nc,nr, precip_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_TOTALPRECIP,source,varfield,vlevel=1,  &
         units="kg/m2")
    call interp_nldas2var(source,nc,nr, tair_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_TAIRFORC,source,varfield,vlevel=1,     &
         units="K")
    call interp_nldas2var(source,nc,nr, swdown_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_SWDOWNFORC,source,varfield,vlevel=1,   &
         units="W/m2")
    call interp_nldas2var(source,nc,nr, lwdown_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_LWDOWNFORC,source,varfield,vlevel=1,   &
         units="W/m2")
    call interp_nldas2var(source,nc,nr, pet_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_PETFORC,source,varfield,vlevel=1,   &
         units="kg/m2")
    call interp_nldas2var(source,nc,nr, cape_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_CAPEFORC,source,varfield,vlevel=1,   &
         units="J/kg")
    ...
    call LVT_logSingleDataStreamVar(LVT_MOC_CRAINFFORC,source,varfield,vlevel=1,   &
         units="kg/m2")
    call interp_nldas2var(source,nc,nr, nwind_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_NWINDFORC,source,varfield,vlevel=1,   &
         units="m/s")
    call interp_nldas2var(source,nc,nr, ewind_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_EWINDFORC,source,varfield,vlevel=1,   &
         units="m/s")
    call interp_nldas2var(source,nc,nr, fheight_f, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_FHEIGHTFORC,source,varfield,vlevel=1,   &
         units="m")
...
! Fixed rootmoist calculation for top 1-meter in NLDAS-2 Mosaic.
! The third layer is 40-200cm depth, so only take 0.375 of it.
! (600mm / 1600mm) - David Mocko
                   rootmoist(c+(r-1)*nc) =&
                        (( 100.0*sml1(c+(r-1)*nc)) +                 &
                         ( 300.0*sml2(c+(r-1)*nc)) +                 &
                         (1600.0*sml3(c+(r-1)*nc)*0.375)) / 1000.0
                endif
             enddo
          enddo
       endif
    ...
    if (nldas2data(source)%smvol) then
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_SOILMOIST,source,varfield,vlevel=3, &
            units="m3/m3")
       if ((nldas2data(source)%lsm.eq."NOAH").or.(nldas2data(source)%lsm.eq."SAC")) then
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_ROOTMOIST,source,varfield,vlevel=1, &
            units="m3/m3")
    ...
    if (nldas2data(source)%lsm.ne."SAC") then
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_SOILTEMP,source,varfield,vlevel=1,  &
            units="K")
    endif
    if ((nldas2data(source)%lsm.eq."NOAH").or.(nldas2data(source)%lsm.eq."VIC")) then
       call interp_nldas2var(source,nc,nr, stl2,  varfield)
       call LVT_logSingleDataStreamVar(LVT_MOC_SOILTEMP,source,varfield,vlevel=2,  &
            units="K")
       call interp_nldas2var(source,nc,nr, stl3,  varfield)
       call LVT_logSingleDataStreamVar(LVT_MOC_SOILTEMP,source,varfield,vlevel=3,  &
            units="K")
    endif
    if (nldas2data(source)%lsm.eq."NOAH") then
       call interp_nldas2var(source,nc,nr, stl4,  varfield)
       call LVT_logSingleDataStreamVar(LVT_MOC_SOILTEMP,source,varfield,vlevel=4,  &
            units="K")
    endif
