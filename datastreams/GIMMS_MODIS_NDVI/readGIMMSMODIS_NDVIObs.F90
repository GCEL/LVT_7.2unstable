...
#endif
  INTEGER    :: i, j, k, ierr
! #endif - cannot be her becuase these integers are needed regardless (DAV)
...
! #endif - I think this is in the wrong place? (DAV)
        endif
     enddo
  enddo
  ...
  if(file_read_status) then 
     ...
     do r = Ydir_numtiles*tile_nr, 1, -1
        ...
        do c = 1, Xdir_numtiles*tile_nc
           ...
           ndvi_inp(c+(i-1)*Xdir_numtiles*tile_nc) = mosaic_ndvi(c,r)
           if(mosaic_ndvi(c,r).ne.LVT_rc%udef) then 
              input_bitmap(c+(i-1)*Xdir_numtiles*tile_nc) = .true.
           endif
        end do
     end do
  endif
  ...
  if(gimmsmodisndviobs(source)%startFlag.and.file_read_status) then 
...
!EOP
  ...
  filename = trim(odir)//'/'//trim(fyr)//'/'//trim(fdoy)//'/'//&
       ...
       trim(fx)//'y'//trim(fy)//'.5v3.NDVI.tif'
